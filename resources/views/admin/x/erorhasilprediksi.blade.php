            ...
            <div class="px-3" id="detail" style="display: none;">
                ...
                <div class="bs-callout bs-callout-info">
                    ...
                    <table class="table table-condensed table-dang">
                        ...
                        <tbody>
                        @foreach ($fuzzifikasi as $n)
                        <tr>
                            <td>
                                @if ($loop->index != 0) 
                                    <code>{{ number_format($pr[$loop->index],5) }}</code>
                                @endif
                            </td>
                        </tr>
                        {{-- <tr>
                            <td>{{ $loop->iteration }}</td>
                            <td><code>{{ $n['bln'] }}</code></td>
                            <td><code>Rp. {{ number_format( $n['laba_bln']) }}</code></td>
                            <td class="text-success">A{{ $n['nilai'] }}</td>
                            <td>
                            @if ($ != 0) 
                            <code>Rp. {{ number_format($n[$loop->index]) }}</code>
                            @endif
                            </td>
                        </tr> --}}
                        @endforeach  
